...
function ProfilePage() {
  ...
  const { user, signInWithGoogle, signInWithApple, signInWithEmail, signOut } = useAuth();
      ...
      {!user && donationsList.length > 0 && (
        <div className='mx-4 mt-4 rounded-2xl p-4 text-white' style={{ background: 'linear-gradient(135deg, #1e6b4e 0%, #2f8f6a 40%, #5ec49a 100%)' }}>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <button
              onClick={async () => { setAuthLoading(true); try { await signInWithApple(); } catch(e) { console.error(e); } finally { setAuthLoading(false); } }}
              disabled={authLoading}
              className='flex items-center justify-center gap-2 w-full py-2.5 bg-black rounded-xl text-sm font-medium text-white hover:bg-gray-900 transition-colors'
            >
            ...
            {!user && (
              <div className='mt-4 space-y-3'>
                ...
                <button
                  onClick={async () => { setAuthLoading(true); try { await signInWithApple(); } catch(e) { console.error(e); } finally { setAuthLoading(false); }}}
                  disabled={authLoading}
                  className='w-full flex items-center justify-center space-x-3 py-3 px-4 bg-black rounded-xl text-white font-medium hover:bg-gray-900'
                >
